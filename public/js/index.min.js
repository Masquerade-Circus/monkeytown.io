(function (global, factory) {
	...
	var Factory = function () {
	    ...
	    return {
	        ...
	        initClient: function initClient(entity) {
	            entity.addScript('fightAnimation', function (dt) {
	                var model = entity.body.getObjectByName('model');
	                if (entity[config$1.PROPS.status] === config$1.STATUS.Fighting) {
	                    var rotationSpeed = 100;
	                    var rotateAngle = Math.PI / 2 * dt * rotationSpeed;

	                    if (THREE.Math.radToDeg(model.rotation.y) > 60) {
	                        var rotateAngle$1 = -THREE.Math.radToDeg(model.rotation.y);
	                        model.rotateY(THREE.Math.degToRad(rotateAngle$1));
	                    } else {
	                        model.rotateY(THREE.Math.degToRad(rotateAngle));
	                    }
	                } else {
	                    var rotateAngle$2 = -THREE.Math.radToDeg(model.rotation.y);
	                    model.rotateY(THREE.Math.degToRad(rotateAngle$2));
	                }
	            });
	...
	var config$1 = {PROPS: PROPS, NET_TYPES: NET_TYPES, ENTITIES: ENTITIES, STATUS: STATUS};
	...
	var Game$1 = {
	    ...
	    updateWorld: function updateWorld(world) {
	        ...
	        for (var i$1 in world.children) {
	             ...
	             */
	            if (Game$1.children[i$1] === undefined && world.children[i$1].id !== Game$1.socket.id) {
	    ...
	    view: function view() {
	        return v( 'article', { oninit: Page$1.onupdate, onupdate: Page$1.onupdate },
	            v( Panel, { position: "center" }, "Welcome, this is the game ui. ", v( 'br', null ),
	                Game$1.player ? ((Game$1.player.p.x) + "/" + (Game$1.player.p.z)) : ''
	            )
	        );
	    }
	};
