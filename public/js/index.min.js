	    ...
	    create: function create(data) {
	        ...
	        entity.update = function update(dt) {
	                ...
	                if (entity.timers.start.length > 0) {
	                    for (var i = 0, l = entity.timers.start.length; i < l; i++) {
	                        entity.timers.start[i](dt);
	                    }
	                }

	                // Run timer handlers
	                for (var i$1 in entity.timers) {
	                    // Update just the time based handlers
	                    if (
	                        entity.timers[i$1].timer &&
	                        Array.isArray(entity.timers[i$1].handlers) &&
	                        entity.timers[i$1].handlers.length > 0 &&
	                        Date.now() - entity.timers[i$1].timer > +i$1
	                    ) {
	                        for (var k = 0, l$1 = entity.timers[i$1].handlers.length; k < l$1; k++) {
	                            entity.timers[i$1].handlers[k](dt);
	                        }
	                        entity.timers[i$1].timer = Date.now();
	                    }
	                }

	                // Run tick handlers
	                if (entity.timers.tick.length > 0) {
	                    for (var i$2 = 0, l$2 = entity.timers.tick.length; i$2 < l$2; i$2++) {
	                        entity.timers.tick[i$2](dt);
	                    }
	                }
	                ...
	                if (entity.timers.end.length > 0) {
	                    for (var i$3 = 0, l$3 = entity.timers.end.length; i$3 < l$3; i$3++) {
	                        entity.timers.end[i$3](dt);
	                    }
	                }
	        ...
	        if (!Entities$1.isNode && Entities$1.Factories[data[props.netType]].Model) {
	        ...
	        if (Entities$1.isNode && typeof Entities$1.Factories[entity[props.netType]].initServer === 'function') {
	            Entities$1.Factories[entity[props.netType]].initServer(entity);
	        }

	        if (!Entities$1.isNode && typeof Entities$1.Factories[entity[props.netType]].initClient === 'function') {
	            Entities$1.Factories[entity[props.netType]].initClient(entity);
	        }
